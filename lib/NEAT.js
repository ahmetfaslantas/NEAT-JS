...
function NEAT(network, popsize, mutationrate, activationfunc) {
	...
	if (popsize === undefined) {
		...
		try { // Try to parse the previously exported network.
			...
			if (network.length === 5) { // Check if the length of the parsed array is valid.
		...
		for (let i = 0; i < this.network.length; i++) { // Check if the network structure is valid.
			if (this.network[i] === undefined || this.network[i] <= 0) {
				throw "Invalid network structure!";
			}
		}


		if (popsize <= 1) { // NEAT algorithm can not work with populations smaller than 1 so we check if that's the case.
		...
		if (activationfunc !== 'sigmoid' && activationfunc !== 'tanh' && activationfunc !== 'relu') { // Check if the given activation function is a valid one if not take action.
			console.error('Invalid activation function has been set!');
			console.error('Setting activation function to relu!');
			activationfunc = 'relu';
		}
	...
	this.predict = function () {
		...
		// Setting all layer's values to 0 except input layer's.
		for (var p = 0; p < this.popsize; p++) {
			for (let i = 1; i < this.network.length; i++) {
				for (let q = 0; q < this.network[i]; q++) {
					this.population[p][1][i][q] = 0;
				}
			}
		}
		...
		for (let p = 0; p < this.popsize; p++) { // Go through every creature.
			for (let layer = 1; layer < this.network.length; layer++) { // Go through every layers from 1 to the last one.
				for (let lnode = 0; lnode < this.network[layer]; lnode++) { // Go through every node.
					...
					this.population[p][1][layer][lnode] = this.applyActivationFunction(this.population[p][1][layer][lnode]);
				}
			}
		}
	...
	this.applyActivationFunction = function (val) {
		if (this.activationfunc === 'sigmoid') return (1 / (1 + Math.exp(val)));
		else if (this.activationfunc === 'tanh') return Math.tanh(val);
	...
	// Remove creature indexed as "index".
	this.removeCreature = function (index) {
		this.population.splice(index, 1);
	}


	// Adds a creature to the this.population array. If declared sets it's weights.
	this.addCreature = function (weight = [], bias_weight = []) {
		// Set all node's values to 0.
		let vals = [];
		for (let i = 0; i < this.network.length; i++) {
			let a = [];
			for (let q = 0; q < this.network[i]; q++) {
		...
		if (weights.length === 0) {
			...
			this.population.push([weights, vals, bias_weights, -1, 0, 0]);
		} else {
			this.population.push([weight, vals, bias_weight, -1, 0, 0]);
		}
	}
