...
function NEAT(network, popsize, mutationrate, activationfunc) {
	...
	if (popsize === undefined) {
		...
		try { // Try to parse the previously exported network.
			...
			if (network.length === 6) { // Check if the length of the parsed array is valid.
				...
				this.geneCount = network[4];
				this.population = network[5];
				this.oldPopulation = [];
			} else {
		...
		for (let y = 0; y < this.popsize; y++) {
			...
			this.population.push([this.weights, this.vals, this.bias_weights, this.prediction, this.fitness, this.score]);
		}

		// Calculate the amount of genes in a creature. Necessary for crossover function.
		for (let q = 0; q < this.population[0][0].length; q++) {
			this.geneCount += this.population[0][0][q].length * this.population[0][0][q][0].length;
		}
	}
	...
	this.predict = function () {
		for (let p = 0; p < this.popsize; p++) {
			for (let layer = 1; layer < this.network.length; layer++) {
				for (let lnode = 0; lnode < this.network[layer]; lnode++) {
					for (let inode = 0; inode < this.network[layer - 1]; inode++) {
						this.population[p][1][layer][lnode] += (this.population[p][1][layer - 1][inode] * this.population[p][0][layer - 1][inode][lnode]) + this.population[p][2][layer - 1][lnode];
						if (this.activationfunc === 'sigmoid') {
							this.population[p][1][layer][lnode] = 1 / (1 + Math.exp(-this.population[p][1][layer][lnode]));
						} else if (this.activationfunc === 'tanh') {
	...
	this.mutate = function () {
		...
		// Go through every creature's every bias weight and randomly change them to a random value.
		for (let i = 0; i < this.popsize; i++) {
	...
	this.exportNet = function () {
		let e = [this.network, this.popsize, this.mutationrate, this.activationfunc, this.geneCount, this.population];
		e = JSON.stringify(e);
		let a = document.createElement("a");
    	let file = new Blob([e], {type: 'text/plain'});
